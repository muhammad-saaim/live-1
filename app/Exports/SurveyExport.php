...
class SurveyExport implements FromView, WithEvents
{
    ...
    {
        // Avoid chunk() inside view; fetch everything at once
        $UserSurveys = UsersSurveysRate::query()
            ->select([
                'users_surveys_rates.id',
                'users_surveys_rates.options_id',
                'users_surveys_rates.users_id',
                'users_surveys_rates.evaluatee_id',
                'users_surveys_rates.survey_id',
                'users_surveys_rates.question_id',
            ...
            ->when($this->survey_id, fn($query) => $query->where('users_surveys_rates.survey_id', $this->survey_id))
            ->orderBy('users_surveys_rates.id')
            ->get(); // 🔄 Replaces chunk()
