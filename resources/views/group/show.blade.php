    ...
    <div class="modal-content border-0 shadow-lg rounded-4" x-data="emailForm()">
      ...
      <form action="{{ route('group.invite') }}" method="POST">
          ...
          <div x-show="dropdownOpen"
            ...
            <template x-for="(email, index) in emails" :key="index">
              <div class="flex justify-between items-center px-4 py-2 hover:bg-gray-100">
                <span x-text="email"></span>
                <button type="button" @click="removeEmail(index)" class="text-red-500 hover:text-red-700 ml-2">×</button>
              </div>
            </template>
          </div>
        </div>
        ...
        <div class="modal-footer border-0 pb-4 px-4">
          ...
          <button type="submit" class="btn btn-primary"><i class="bi bi-send me-1"></i> Invite</button>
        </div>
      </form>
    </div>
  </div>
