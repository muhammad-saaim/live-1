        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('group.update', $group->id) }}" method="POST" id="edit-group-form">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="name" class="block text-sm font-medium text-gray-700">Group Name</label>
                            <input type="text" name="name" id="name" value="{{ old('name', $group->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('description', $group->description) }}</textarea>
                        </div>
                                    ...
                                    <div>
                                        ...
                                        <label class="inline-flex items-center me-4">
                                            <input type="checkbox" name="group_types[]" value="{{ $groupType->id }}"
                                                ...
                                                {{ in_array($groupType->id, old('group_types', $group->groupTypes->pluck('id')->toArray())) ? 'checked' : '' }}>
                                                ...
                                                @foreach($groupType->children as $child)
                                                    <label class="inline-flex items-center">
                                                        <input type="checkbox" name="subgroup_types[]" value="{{ $child->id }}"
                                                            ...
                                                            {{ in_array($child->id, old('subgroup_types', $group->groupTypes->pluck('id')->toArray())) ? 'checked' : '' }}>
