            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('question.update', $question->id) }}" method="POST">
                        ...
                        <h2 class="text-lg font-semibold text-gray-700 mb-4">Survey : {{ $question->survey->title ?? 'N/A' }}</h2>
                        <div class="mb-4">
                            ...
                            <textarea name="question" id="question" rows="3" maxlength="250" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300
                                        focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('question', $question->question) }}
                            </textarea>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="points" id="points" value="{{ old('points', $question->points) }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="is_active" id="is_active" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="1" {{ old('is_active', $question->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $question->is_active) == 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
